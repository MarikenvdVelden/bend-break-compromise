---
title: "Pre-Analysis Plan: Who Bends and Who Breaks? Experimental Evidence for Conditions Affecting Voters' Evaluations of Politicians Striking Compromises"
...
**Gender hypothesis** (_H1_):  Women politicians are evaluated lower when striking a political compromise compared to men politicians.
...
**Intersection hypothesis** (_H3_): Women politician with a migration background are evaluated lower when striking a political compromise compared to men politicians without a migration background.
...
depVarList <- setdiff(colnames(df), c("gender", "compromise"))
allModels <- lapply(depVarList, function(x){
  lm(formula= paste0("`", x, "` ~ gender * compromise"), 
     data= df, na.action = na.omit)})

for(i in 1:length(depVarList)){
  if(i==1){
    m <- summary(margins(allModels[[i]]), at = list(compromise = 1)) %>%
      filter(factor=="gender") %>%
    ...
    tmp <- summary(margins(allModels[[i]]), at = list(compromise = 1)) %>%
      filter(factor=="gender") %>%
      mutate(y = depVarList[i],
             lower = AME - (1.645 * SE),
             upper = AME + (1.645 * SE)) %>%
...
depVarList <- setdiff(colnames(df), c("migration", "compromise"))
allModels <- lapply(depVarList, function(x){
  lm(formula= paste0("`", x, "` ~ migration * compromise"), 
...
depVarList <- setdiff(colnames(df), c("intersection", "compromise"))
allModels <- lapply(depVarList, function(x){
  lm(formula= paste0("`", x, "` ~ intersection * compromise"), 
     data= d, na.action = na.omit)})

for(i in 1:length(depVarList)){
  if(i==1){
    m <- summary(margins(allModels[[i]]), at = list(compromise = 1)) %>%
      filter(factor=="intersection") %>%
      mutate(y = depVarList[i],
             lower = AME - (1.645 * SE),
             upper = AME + (1.645 * SE)) %>%
      select(AME, upper, lower, y)

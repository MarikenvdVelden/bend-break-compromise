regression_explore <- function(df, a, b, compromise, issue){
  ...
  indepVarList <- df %>% select(a, b, compromise, F2, F4, F7, F8, F9,
                                PreT1:PreT7_5, PreT7_7:PreT8, issue)   
...
df <- d %>% select(a = gender, b = PreT7_6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT7_5, 
                   PreT7_7:PreT8, issue) 
...
p1e <- e1 %>%
  ...
  theme_minimal() +
  labs(x = "Willingness to Accept Compromise\n Not at all (1) - Very much (5)", 
...
df <- d %>% select(a = immigration, b = PreT7_6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT7_5, 
                   PreT7_7:PreT8, issue) 
...
df <- d %>% select(a = intersection, b = PreT7_6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT7_5, 
                   PreT7_7:PreT8, issue) 

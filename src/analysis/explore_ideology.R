regression_explore <- function(df, a, b, compromise, issue){
  ...
  indepVarList <- df %>% select(a, b, compromise, F2, F4, F7, F8, F9, 
                                PreT1:PreT5, PreT7_1:PreT8, issue)   
...
df <- d %>% select(a = gender, b = PreT6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT5, 
                   PreT7_1:PreT8, issue) 
...
p1e <- e1 %>%
  ...
  theme_minimal() +
  labs(x = "Political Ideology\n Left (0) - Right (10)", y = "Average Marginal Effects of Being a Women Politician",
       title = "Gender Hypothesis") +
  facet_grid(y~compromise) +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_hline(yintercept = 0, size = .2, linetype = "dashed") 

df <- d %>% select(a = immigration, b = PreT6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT5, 
                   PreT7_1:PreT8, issue) 
...
p2e <- e2 %>%
  ...
  geom_ribbon(fill = fig_cols[1], alpha = .4) +
  theme_minimal() +
  labs(x = "Political Ideology\n Left (0) - Right (10)", y = "Average Marginal Effects of Being a Politician with a Migration Background",
       title = "Migration Hypothesis") +
  facet_grid(y~compromise) +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_hline(yintercept = 0, size = .2, linetype = "dashed") 

df <- d %>% select(a = intersection, b = PreT6, compromise, 
                   PT1:PT4, F2, F4, F7, F8, F9, PreT1:PreT5, 
                   PreT7_1:PreT8, issue) 
...
p3e <- e3 %>%
  ...
  geom_ribbon(fill = fig_cols[1], alpha = .4) +
  theme_minimal() +
  labs(x = "Political Ideology\n Left (0) - Right (10)", y = "Average Marginal Effects of Being a Women Politician with a Migration Background",

d <- d %>%
  ...
  pivot_longer(cols = c(compromise_D, compromise_I, compromise_E, compromise_C),
               ...
               names_to = "issue_c") %>%
  mutate(issue_c = recode(issue_c, 
                          compromise_C = "COVID",
                          compromise_D = "Defense",
                          compromise_E = "Education",
                          compromise_I = "Immigration")) %>%
  ...
  mutate(issue4 = recode(issue4, 
                         ...
                         PT_E4 = "Education",
                         PT_I4 = "Immigration"),
         check = if_else(issue == issue_c & issue == issue1 & issue == issue2 & issue == issue3 & issue == issue4,
                         1, 0)) %>%
  filter(check == 1) %>%
  mutate(gender = recode(name,
         ...
         intersection = recode(name,
                               ...
                               `Rachid Amezian` = 0,
                               `Rachida Amezian` = 1),
         compromise = if_else(compromise == "geen compromis", 0, 1),
